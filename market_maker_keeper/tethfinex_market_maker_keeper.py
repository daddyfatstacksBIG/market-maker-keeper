...
from pyexchange.tethfinex import TEthfinexApi
from pyexchange.tethfinex import TEthfinexToken
...
class TethfinexMarketMakerKeeper:
    ...
    def __init__(self, args: list, **kwargs):
        ...
        self.token_sell_wrapper = TEthfinexToken(
            self.web3, Address(token_registry[token_sell]["wrapperAddress"]), token_sell
        )
        self.token_buy_wrapper = TEthfinexToken(
            self.web3, Address(token_registry[token_buy]["wrapperAddress"]), token_buy
        )
    ...
    def deposit_for_sell_order(self, missing_sell_amount: Wad):
        ...
        if missing_sell_amount > Wad(0):
            receipt = self.token_sell_wrapper.deposit(missing_sell_amount).transact(
    ...
    def deposit_for_buy_order(self, missing_buy_amount: Wad):
        ...
        if missing_buy_amount > Wad(0):
            receipt = self.token_buy_wrapper.deposit(missing_buy_amount).transact(
                gas_price=self.gas_price
            )
            return receipt is not None and receipt.successful
        else:
